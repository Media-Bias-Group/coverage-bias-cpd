...
class Trainer(object):
    def __init__(self, args):
        ...
        self.logging = create_exp_dir(args.exp_dir) # create experiment directory and logging function
    ...
    def load_data(self):
        ...
        #print(f"Train: {self.train_loader.shape}, Valid: {self.valid_loader.shape}, Test: {self.test_loader.shape}")
        off_diag = np.ones([args.num_atoms, args.num_atoms]) - np.eye(args.num_atoms)
        print(f"shape off_diag: {off_diag.shape}")
        rel_rec = np.array(encode_onehot(np.where(off_diag)[1]), dtype=np.float32) # receiver matrix
        rel_send = np.array(encode_onehot(np.where(off_diag)[0]), dtype=np.float32) # sender matrix -> used in message-passing GNNs to efficiently gather sender and receiver node features for every edge
    ...
    def train(self):
        ...
        for epoch in range(self.args.epochs):
            t = time.time()
            mse_train, delta_train, acc_train = self.train_one_epoch() #mse_train, delta_train, acc_train
    ...
    def train_one_epoch(self):
        ...
        for batch_idx, (data, relations, cpd) in enumerate(self.train_loader): # each batch 
            #print(f"Batch {batch_idx}")
            data, relations = data.to(self.device), relations.to(self.device) # cpd is not used: unsupervised # data [batch_size, num_atoms, num_timesteps, num_dims]
            #print(f"data shape: {data.shape}")
            data = data[:, :, :self.args.timesteps, :] # data up to the training time step
            self.model.optimizer.zero_grad()
            logits = self.model.encode(data, self.rel_rec, self.rel_send) #rec and send are receiver and sender matrices
            ...
            output = self.model.decode(data, edges, self.rel_rec, self.rel_send) # decoder for the variable dynamics
            #print(output.shape)
    ...
    def evaluate(self):
        ...
        cpds = np.array(cpds)
        #avg_roc, avg_dist, avg_tri = cpd_metrics(probs, cpds) #changed for the real dataset
        if self.data_type == "sim":
        ...
        if self.report_combine:
            ...
            print(f"recons shape: {recons.shape}, origs shape: {origs.shape}")
            type1_score = mse_anomaly(recons, origs, step=1)
