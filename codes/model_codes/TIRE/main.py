...
samoa = pd.read_csv("samoa_timeseries.csv")

# preprocess samoa data
samoa = samoa.drop(columns=["MonthYear"])
samoa = 2 * (samoa - samoa.min()) / (samoa.max() - samoa.min()) - 1

timeseries = samoa.to_numpy(dtype=float)   # shape: (T, D)
windows_TD_3d = utils.ts_to_windows(timeseries, 0, window_size, stride=1)
windows_TD = windows_TD_3d.reshape(windows_TD_3d.shape[0], -1)
windows_FD_parts = []
for d in range(timeseries.shape[1]):
    fd_d = utils.calc_fft(windows_TD_3d[:, :, d], nfft, norm_mode)
    windows_FD_parts.append(fd_d)
windows_FD = np.concatenate(windows_FD_parts, axis=1)
