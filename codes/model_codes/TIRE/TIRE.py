...
def create_parallel_aes(window_size_per_ae,
    ...
    z = tf.keras.layers.Concatenate(axis=-1)([z_shared, z_unshared])
    ...
    def pae_loss(x,x_decoded):
        ...
        #squared_diff = keras.ops.square(x-x_decoded)
        mse_loss = tf.reduce_mean(squared_diff)
        #mse_loss = keras.ops.mean(squared_diff)
        
        square_diff2 = K.square(z_shared[:,1:,:]-z_shared[:,:nr_ae-1,:])
        #square_diff2 = keras.ops.square(z_shared[:,1:,:]-z_shared[:,:nr_ae-1,:])
        shared_loss = tf.reduce_mean(square_diff2)
        #shared_loss = keras.ops.mean(square_diff2)
...
def train_AE(windows, intermediate_dim=0, latent_dim=1, nr_shared=1, nr_ae=3, loss_weight=1, nr_epochs=200, nr_patience=200, summary=True, verbose=1):
    """
    ...
    pae, encoder, decoder = create_parallel_aes(window_size_per_ae,intermediate_dim,latent_dim,nr_ae,nr_shared,loss_weight,summary=summary)
...
def smoothened_dissimilarity_measures(encoded_windows, encoded_windows_fft, domain, window_size):
    ...
    if domain == "TD":
        ...
        #print(encoded_windows)
        #print(encoded_windows_fft)
        #print(encoded_windows_both)
...
def change_point_score(distances, window_size):
    ...
    max_prominence = np.amax(prominences)
    prominences = prominences/max_prominence if max_prominence > 0 else prominences  # avoid 0/0 when the dissimilarity signal is flat (no detectable change point)
